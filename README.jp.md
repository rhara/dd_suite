...
`dd_draw` / `dd_idea` / `dd_mdstability` / `dd_molview` / `dd_overlay` / `dd_prep` /
`dd_seqalign` の各レポジトリを、コードやenvを統合することなく**1つのsuite**として使える
...
12個すべてを再生する——何も新しく発明はしていない、単なる自動化である:
...
- **`dd_molview`** はpipパッケージではなく、コンパイル済みのC++/Qt6
  ...
  参照すること。**Intel Mac（`osx-64`）ではQt6をconda-forgeから調達できない**
  ——そのプラットフォーム向けの`qt6-webengine`ビルドが存在しないため
  （`osx-arm64`、`linux-64`、`win-64`のみ存在）、`install_all.py`は
  `osx-64`を検知すると`qt6-main`/`qt6-webengine`だけをそこで作成する
  envから除外する（`rdkit`/`biopython`などは通常通りインストールされる）。
  Intel MacでのQt6回避策（Homebrewベースのビルド）については
...
| `dd_draw` | `rdkit numpy jinja2 reportlab svglib pytest` | PDF生成はWeasyPrint/cairosvgではなくreportlab+svglib（純Python）——システムのCairo/Pango/GTK依存を避けるため。詳細は`dd_draw/README.md`の「Why reportlab+svglib」参照 |
...
| `dd_idea` | `biopython pandas numpy matplotlib py3dmol streamlit pymol-open-source fpocket` | `fpocket`は外部CLIバイナリ |
| `dd_molview` | `rdkit biopython pandas numpy py3dmol pybind11 pytest qt6-main qt6-webengine` | C++/Qt6ビルド——env作成は自動化されているが、`cmake -S . -B build && cmake --build build`ステップは自動化されていない（`dd_molview/README.md`参照）。**Intel Mac（`osx-64`）では**`install_all.py`がこのリストから`qt6-main`/`qt6-webengine`を除外する（conda-forgeに同プラットフォーム向けビルドが無いため）——Qt6は別途Homebrewから調達する必要あり（`dd_molview/README.md`参照） |
...
`dd_prep`、`dd_docking`、`dd_mdstability`、`dd_draw`）内で実行される——
`dd_suite`自体はそれらのコードにも依存関係にも一切触れない。`dd_draw`は
`dd_confhunt`と同様、単一コマンドのプロジェクト（動詞サフィックス無し）
——コマンド名とenv名がどちらも単に`dd_draw`。
...
| `scripts/install_all.py` | 12プロジェクト全部のenvを一括構築するスタンドアロンインストーラー（前述の「dd_*プロジェクトを一括インストールする」参照）——`dd_suite`パッケージ自体には含まれない、標準ライブラリのみのスクリプト |
...
- `install_all.py`は12プロジェクト中11個についてenv作成+editable
